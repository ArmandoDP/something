    ...
    <section>
        <form method="POST" action="{{ route('ventas.update', $venta->id) }}">
            ...
            <select name="producto_id" id="producto_id" onchange="actualizarPrecio()">
                ...
                <option value="{{ $producto->id }}" data-precio="{{ $producto->precio }}" {{ $producto->id == $venta->producto_id ? 'selected' : '' }} >{{ $producto->nombre }}
                </option>
                @endforeach
            </select>
            ...
            <select name="vendedor_id" id="vendedor_id">
                ...
                <option value="{{ $vendedor->id }}" {{ $vendedor->id == $venta->vendedor_id ? 'selected' : '' }}>{{ $vendedor->nombre }}</option>
                @endforeach
            </select>

            <label for="cantidad">Cantidad:</label>
            <input type="number" name="cantidad" id="cantidad" value="{{ $venta->cantidad }}" oninput="calcularTotal()">

            <label for="precio">Precio:</label>
            <input type="text" name="precio" value="{{ $venta->producto-> precio}}" id="precio" readonly>

            <label for="total">Total:</label>
            <input type="number" name="total" value="{{ $venta->total }}" id="total" readonly>
